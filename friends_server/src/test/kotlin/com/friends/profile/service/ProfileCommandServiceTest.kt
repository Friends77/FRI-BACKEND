        ...
        given("updateProfile 메서드를 호출할 때") {
            ...
            `when`("존재하는 프로필을 수정하면") {
                ...
                then("수정된 프로필이 저장되어야 한다.") {
                    verify(exactly = 1) {
                        ...
                        s3ClientService.upload(any(), any()) // 새로운 이미지 업로드
                    }
                    ...
                    existingProfile.imageUrl shouldBe TEST_IMAGE_UPLOAD_FILE_URL // 이미지가 업데이트 되었는지 확인
                }
            }
            ...
            `when`("기존 프로필 이미지가 존재하지 않으면") {
                ...
                then("이미지가 삭제되지 않아야 한다.") {
                    verify(exactly = 0) { s3ClientService.deleteS3Object(any()) } // 삭제 로직 실행되지 않음
                    verify(exactly = 1) { s3ClientService.upload(any(), any()) } // 새로운 이미지 업로드
                    existingProfile.imageUrl shouldBe TEST_IMAGE_UPLOAD_FILE_URL // 이미지가 업데이트 되었는지 확인
                }
            }

            `when`("이미지 수정이 없을 경우(updateDto의 imageUrl이 null이 아닌 경우)") {
                profileCommandService.updateProfile(MEMBER_ID, updatedProfile, null)
                then("기존 이미지가 유지되어야 한다.") {
                    verify(exactly = 0) { s3ClientService.deleteS3Object(any()) } // 삭제 로직 실행되지 않음
                    verify(exactly = 0) { s3ClientService.upload(any(), any()) } // 새로운 이미지 업로드 로직 실행되지 않음
                    existingProfile.imageUrl shouldBe TEST_IMAGE_URL // 이미지가 업데이트 되지 않았는지 확인
                }
            }

            `when`("updateDto의 imageUrl이 null일 경우") {
                profileCommandService.updateProfile(MEMBER_ID, updateTestProfile(imageUrl = null), null)
                then("이미지가 기본으로 변경되어야 한다.") {
                    verify(exactly = 1) { s3ClientService.deleteS3Object(any()) } // 삭제 로직 실행되지 않음
                    verify(exactly = 0) { s3ClientService.upload(any(), any()) } // 새로운 이미지 업로드 로직 실행되지 않음
                    existingProfile.imageUrl shouldBe null
                }
            }

            `when`("updateDto의 imageUrl이 null이면서 기존 이미지가 존재하는 경우") {
                existingProfile.imageUrl = null
                profileCommandService.updateProfile(MEMBER_ID, updateTestProfile(imageUrl = null), null)
                then("삭제 로직이 실행되지 않고 기본 이미지로 변경되어야한다.") {
                    verify(exactly = 0) {
                        s3ClientService.deleteS3Object(any())
                        s3ClientService.upload(any(), any())
                    }
                    existingProfile.imageUrl shouldBe null
                }
            }

            ...
            given("extractMemberMostAgeRange") {
                ...
                `when`("2개 이상 나이대가 가장 많은 경우") {
                    ...
                    then("가장 많은 나이대를 모두 저장한다.") {
                        ...
                        verify(exactly = 1) {
                            chatRoomMemberAgeRangeRepository.save(chatRoom.id, "20,30,40")
                        }
                    }
                }
                ...
                `when`("특정 나이대가 가장 많은 경우") {
                    every { chatRoomMemberRepository.findByChatRoom(any()) } returns
                        listOf(
                            createTestChatRoomMember(chatRoom = chatRoom, member = createTestMember(nickname = "test1", email = "test1", profile = createTestProfile(birth = LocalDate.now().minusYears(10).minusDays(1))), lastReadMessage = createTestMessage()), //10세
                            createTestChatRoomMember(chatRoom = chatRoom, member = createTestMember(nickname = "test2", email = "test2", profile = createTestProfile(birth = LocalDate.now().minusYears(10).plusDays(1))), lastReadMessage = createTestMessage()), // 9세
                    ...
                    then("가장 많은 나이대를 저장한다.") {
                        chatRoomRecommendationCriteriaService.extractMemberMostAgeRange()
                        verify(exactly = 1) {
                            chatRoomMemberAgeRangeRepository.save(chatRoom.id, "10")
                        }
                    }
                }
            ...
            given("extractMemberMostGender") {
                ...
                `when`("남자와 여자 수가 같을 때") {
                    ...
                    then("남자와 여자를 저장한다.") {
                        ...
                        verify(exactly = 1) {
                            chatRoomMemberGenderRepository.save(chatRoom.id, "MAN,WOMAN")
                        }
                    }
                }
                ...
                `when`("남자가 더 많을 때") {
                    ...
                    then("남자를 저장한다.") {
                        chatRoomRecommendationCriteriaService.extractMemberMostGender()
                        verify(exactly = 1) {
                            chatRoomMemberGenderRepository.save(chatRoom.id, "MAN")
                        }
                    }
                }

                `when`("아무런 데이터가 없을 때") {
                    every { chatRoomMemberRepository.findByChatRoom(any()) } returns emptyList()
                    then("저장하지 않는다.") {
                        chatRoomRecommendationCriteriaService.extractMemberMostGender()
                        verify(exactly = 0) {

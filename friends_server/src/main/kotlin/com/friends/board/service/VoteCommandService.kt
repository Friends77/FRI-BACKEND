...
class VoteCommandService (
     ...
     * 투표 선택지 추가하기
     */
    fun addVoteOption(voteId: Long, optionContent: String): VoteOption {
     ...
     * 투표 선택지 삭제하기
     */
    fun deleteVoteOption(voteId: Long, optionId: Long){
        val vote = voteRepository.findById(voteId)
            .orElseThrow{VoteNotFoundException()}
        val optionToDelete = vote.options.find { it.id == optionId }
    ...
    fun incrementVoteCount(optionId: Long){
        val option = voteRepository.findOptionById(optionId)
            ?: throw VoteOptionNotFoundException()
        option.voteCount+=1

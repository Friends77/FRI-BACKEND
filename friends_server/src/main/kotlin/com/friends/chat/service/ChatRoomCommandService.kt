...
class ChatRoomCommandService(
    ...
    fun updateChatRoom(
        ...
        val changeChatRoomInfo = updateChatRoomTitle(chatRoom, request)
    ...
    private fun updateChatRoomTitle(
        chatRoom: ChatRoom,
        request: ChatRoomUpdateRequestDto?,
    ): Boolean {
        ...
        // 채팅방의 카테고리 중 없는 카테고리 ID 필터링해서 해당 카테고리 ID에 해당하는 카테고리 리스트 가져오기
        val addCategoryList =
            request
                ?.addCategoryIds
                ?.filter { it !in chatRoom.categories.map { c -> c.id } }
                ?.toSet()
        ...
        val removeCategory = chatRoom.categories.filter { it.id in (request?.removeCategoryIds ?: emptySet()) }
        if (removeCategory.isNotEmpty()) {
            chatRoomCategoryRepository.deleteAll(removeCategory)
            changeChatRoomInfo = true
        }

        if (chatRoom.categories.isEmpty()) {
